...
document.addEventListener('DOMContentLoaded', function () {
  ...
  addData();
        ...
        function populateDropdown() {
			...
			const BILL_ID = document.getElementById("BILL_ID");
			const status = document.getElementById("status");

            dateList.innerHTML = "";

            Object.keys(dateData).forEach((date, index) => {
                ...
                if (index === 0) {
                    selectedDate.innerHTML = `${date} <i data-feather="chevron-down" aria-hidden="true"></i>`;
                    total.textContent = `Total: ${dateData[date].total}`;
                    revenue.textContent = `Revenue: ${dateData[date].revenue}`;
                    success.textContent = `Success: ${dateData[date].success}`;
                    error.textContent = `Error: ${dateData[date].error}`;
					BILL_ID.textContent = `BILL_RUN_ID: ${dateData[date].BILL_ID} ${dateData[date].status}`;
                }
